...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "SELECT ninja.id, ninja.name, player.status, player.XacThuc, player.coin, player.luong, ninja.vip, player.nap, player.ip_address, player.password, player.phone
            FROM ninja 
            JOIN player ON ninja.id = player.id
...
<?php include('error.php'); ?>
        ...
        <div id="result-container">
            ...
            if (isset($result)) {
                if ($result->num_rows > 0) {
                    ...
                    while ($row = $result->fetch_assoc()) {
                        ...
                        echo "<tr class='fw-semibold'><td>Trạng thái</td><td>" . getActivatedDescription($row["XacThuc"]) . "</td></tr>";
                        ...
                        echo "<tr class='fw-semibold'><td>VIP</td><td>" . $row["vip"] . "</td></tr>";
                        }                        
           ...
           //                echo "<a class='cursor-pointer text-primary show-all-ip' style='display:$showAllLinkStyle;'><span> </span>Hiển thị Tất cả</a>";
                        ...
                        echo "</td></tr>";
                        echo "<tr class='fw-semibold'><td>Đã Nạp</td><td>" . number_format($row["nap"]) . "</td></tr>";
